...
;;; feedmail works with recent versions of GNUemacs (20.x series) and
;;; XEmacs (tested with 20.4 and later betas).  It probably no longer
;;; works with GNUemacs v18, though I haven't tried that in a long
...
;; Author: WJCarpenter <bill@carpenter.ORG>, <bill@bubblegum.net>
;; Version: 10
...
;;
;; LCD record:
;; feedmail|WJCarpenter|bill@carpenter.ORG,bill@bubblegum.net|Outbound mail queue handling|01-04-22|10|feedmail.el
;;
;; Change log:
...
internal buffers will be reused and things will get confused.  It's
not necessary to arrange for the undoing of any changes you make to
the buffer."
  :group 'feedmail-misc
  :type 'hook
)
...
If you use the binmail form, check the value of
...
Feeds the buffer to it.  Probably has some flaws for RESENT-* and
other complicated cases.  Takes addresses from message headers and
...
;;					 (insert-file-contents maybe-file)))
			   ...
			   (feedmail-say-chatter "%d to go, %d sent, %d skipped (%d other files ignored)"
...
(defun feedmail-queue-send-edit-prompt-inner (default prompt reprompt helper standard-alist user-alist)
  ...
  (let ((filename)
	...
	(if buffer-file-name
		(progn
		  ...
		  (setq is-in-this-dir (string-equal
								(directory-file-name (expand-file-name queue-directory))
	...
	(if (and previous-buffer-file-name (or (not is-fqm) (not is-in-this-dir))
			 (let (d b s)
			   ...
			   (y-or-n-p (format "FQM: Was previously %s%s%s; delete that? " d s b))
			   )
			 )
    ...
    (unwind-protect
		(save-excursion
		  ...
		  (let ((case-fold-search nil))
			...
			(feedmail-say-debug "looking for m-h-s \"%s\"" mail-header-separator)
...
feedmail-fiddle-plex-blurb."
  (feedmail-say-debug ">in-> feedmail-fiddle-header %s %s %s %s" name value action folding)
  (let ((case-fold-search t)
  ...
  (save-excursion
	(if feedmail-enable-spray
		(mapcar
		 ...
		 feedmail-address-list)
	  (feedmail-say-debug "calling buffer-eater %s" feedmail-buffer-eating-function)
	  (funcall feedmail-buffer-eating-function
			   feedmail-prepped-text-buffer
			   feedmail-error-buffer
			   feedmail-address-list))))
...
probably not appropriate for you."
  (feedmail-say-debug ">in-> feedmail-default-message-id-generator %s" maybe-file)
  (let ((date-time)
		(system-time-locale "C")
...
(defun feedmail-default-x-mailer-generator ()
  ...
  (feedmail-say-debug ">in-> feedmail-default-x-mailer-generator")
  (concat
...
headers, including the intervening newlines."
  (feedmail-say-debug ">in-> feedmail-accume-n-nuke-header %s %s" header-end header-regexp)
